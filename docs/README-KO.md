...
- 앱 전체 글자 크기(Settings)와, TXT·Markdown·HTML 문서 안에서 따로 저장되는
  본문 확대/축소가 있습니다. 글꼴은 하우스 스타일에 따라 Lexend→Verdana로
  고정되어 있어 서체 자체는 바꿀 수 없습니다.
- 텍스트를 선택해 하이라이트하고 메모를 붙이면 선택한 원문 그대로 저장되어
  나중에 목록·내보내기에서 그대로 다시 볼 수 있습니다.
- 선택 없이 `Add note here`로 문서 전체 또는 특정 구간(가까운 소제목)에 대한
  메모도 남길 수 있습니다. 자세한 사용법은
  [USER-GUIDE-KO.md](USER-GUIDE-KO.md)의 "선택·하이라이트·메모" 절을
  참고하세요.
