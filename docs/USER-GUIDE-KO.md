...
- `Read aloud`는 선택한 문구 중 **영문 부분만** 기기의 음성 합성(Web Speech
  API, 인터넷 연결 불필요)으로 읽어 줍니다. 한글·영문이 섞인 문장에서도 영문만
  추려 읽습니다. 재생 중에는 버튼이 `Stop`으로 바뀌며, 다시 누르거나 다른
  텍스트를 선택하거나 문서를 닫으면 재생이 멈춥니다. 브라우저가 음성 합성을
  지원하지 않으면 버튼이 표시되지 않습니다.
