...
### 뒤이은 자체 재검토(같은 날) — 목록 중복 그리기 방지 (빌드 `2026.09.18-batchselect2`)

- **잠재적 문제**: 선택 모드를 끄는 새로고침과 삭제·이동 뒤의 새로고침이 연달아 실행되면, 두 호출이
  서로 목록을 비운 뒤 각자 그려서 같은 문서 줄이 두 번 나타날 수 있는 구조였다(기존 코드에도 있던
  구조이나 새 기능이 호출 빈도를 높임).
- **수정**: `refreshLibrary()`에 호출 번호(ticket)를 두어, 최신이 아닌 호출은 화면을 건드리기 전에 중단.
- **재확인**: 5건 전체 선택 → `Delete` → `Undo` 흐름에서 줄 수가 5 → 0 → 5로 정확함. 글자 크기 6px·17px에서
  선택 바 버튼 전부 44px 높이·잘림 없음, 다크 모드의 `Custom order` 시트 버튼 44×44px. `npm test` 100/100 통과.
- 백업 파일의 문서 레코드가 `sortOrder`를 그대로 담는다는 것을 `src/backup.js` 코드로 확인(안내 문서 내용과 일치).


---

## 2026-09-18 — 붙여넣기 문서 추가(Paste code) · 폴더 관리 노출 · 첫 화면 개선 (빌드 `2026.09.18-pastefolders1`)

**요청**: ① 파일 가져오기가 아니라 코드·글을 직접 입력해서도 문서를 추가할 수 있게, ② 첫 화면을 더
유용하게, ③ 폴더 이름 수정·삭제가 가능해야 함.

**먼저 확인한 사실**

- 폴더 **이름 수정·삭제 기능은 코드에 이미 있었고 정상 동작**했다(브라우저에서 생성 → 이름 변경 → 문서 이동 →
  삭제 → 문서 4건이 `Unsorted`로 복귀까지 확인). 문제는 기능이 아니라 **발견성**이었다 — 관리 진입점이
  폴더 추가 버튼처럼 보이는 `+ Folders` 칩 하나뿐이고, 그 안의 시트에 `Rename`/`Delete`가 숨어 있었다.
- 작업 시작 시 `Published/folio` 작업 트리의 3개 파일(`sw.js`, `src/version.js`, `assets/app.css`)이 이미
  배포된 최신 커밋(HEAD)보다 오래된 9/16 커밋 상태로 git 밖에서 덮어써져 있었다(바이트 단위로 9/16 커밋과
  동일 — 잃는 고유 내용 없음). 그대로 작업하면 배치 선택·Custom order 스타일이 빠진 채 배포될 위험이 있어,
  사용자 확인을 받고 HEAD로 복원한 뒤 작업했다(옛 사본은 작업 임시 폴더에 보관).

**변경**

- **`Paste code`**(신규 `src/compose.js`): 아래쪽 바와 빈 화면의 버튼으로 `Add from text` 창을 연다.
  종류 `Markdown`/`Text`/`HTML`/`CSV`(HTML 코드는 자동 감지), 제목 자동 추출(HTML `<title>`·`<h1>`, Markdown
  머리말 `title:`·첫 제목, 그 외 첫 줄), 폴더 선택(열려 있는 폴더가 기본), `Paste from clipboard`, 글자 수·크기
  표시, 입력 임시 보존(메모리만). 입력한 글을 `File`로 감싸 **기존 가져오기 파이프라인(`importFiles`)에 그대로**
  넘기므로 감지·해시·중복 처리·검색 색인·저장 공간 확보가 파일 가져오기와 동일하다.
- **폴더 바**: 폴더를 열면 탭 아래에 이름과 `Rename`·`Delete`가 바로 나타난다. `+ Folders` 칩은 **`+ New folder`**
  (점선 칩)로 바뀌어 폴더를 만들고 곧바로 연다. `⋯ → Folders` 시트는 그대로 두되 같은 함수(rename/delete 흐름)를 쓴다.
- **폴더 색**(무지개 확장 팔레트, 사용자 승인): 폴더마다 5색(주황·노랑·초록·하늘·보라) 중 하나. 대표색은 핑크
  유지. 색은 폴더 탭·폴더 바·문서 줄의 폴더 배지에만 쓴다. 색 번호는 폴더 레코드의 `hue`에 저장(먼저 만든 5개는 서로
  다른 색, 백업·복원에 포함, 없으면 폴더 id에서 정해진 값).
- **목록 요약 줄**: 상태 칩·종류/태그 필터·검색으로 목록이 줄어 있으면 `Showing 3 of 12` + `Clear filters`.
- **빈 화면**: `Import files`와 `Paste code` 두 버튼. 비어 있는 폴더는 `“이름” is empty` 안내와 같은 두 버튼.
- **폴더가 열려 있을 때의 가져오기**: `Import files`·끌어다 놓기·`Paste code`로 넣은 문서는 그 폴더에 저장
  (`Added 2 documents to 업무.`). 이전에는 항상 `Unsorted`로 들어가 열어 둔 폴더 목록에서 사라져 보였다.
- **Sync 정합성**: 폴더 이름을 바꾸면 그 폴더 문서의 `updatedAt`을 갱신하고 Sync를 예약한다. 다른 기기는
  `updatedAt`이 더 새로울 때만 폴더 변경을 반영하므로, 이전에는 이름을 바꿔도 다른 기기에 전달되지 않았다.
  삭제도 Sync를 예약한다.
- `ui.js`: 시트가 시작 입력칸을 `data-autofocus`로 지정할 수 있게 함(기본 동작은 그대로).

**고친 문제 (작업 중 발견)**

1. **[테스트]** `tests/activity-session.test.mjs`의 "background then resume" 테스트가 **매일 자정 직전 약 2시간
   20분 동안(로컬·CI 모두) 실패**하는 시간대 의존 결함. 시계를 실제 "지금"에서 시작해 몇 시간씩 진행시키는데,
   자정을 넘으면 세션 추적기가 (정상적으로) 세션을 둘로 쪼개서 기대값과 어긋난다. 시작 시각을 "어제 오전 10시"로
   고정해 수정. 수정 전 기준선은 100개 중 99개 통과였다.
2. **[신규 기능]** 직전에 HTML을 저장하면 다음에 붙여넣은 **Markdown이 HTML로 저장**되던 결함(마지막 종류를
   기본값으로 기억하면서 HTML까지 기억). HTML은 기억하지 않고 글 내용으로만 판정하도록 수정, 회귀 테스트 추가.
3. **[신규 기능]** `#`나 `---`(YAML 머리말 구분선)만 있는 줄이 제목이 되던 문제 → 기호만 있는 줄은 건너뛰고,
   머리말의 `title:`을 읽도록 수정.
4. **[기존 결함]** 새 폴더의 `order`를 `folders.length`로 잡아, 폴더를 지운 뒤 만들면 순서 값이 겹쳐 새 폴더가
   탭 줄 **끝이 아니라 중간**에 끼어들던 문제 → `가장 큰 값 + 1`로 수정, 회귀 테스트 추가.
5. **[신규 기능]** 새 폴더 바가 탭 줄 바로 아래에 붙어 두 줄 칩의 투명 터치 영역(각 44px)이 겹쳐 `+ New folder`의
   실제 탭 높이가 40px로 줄던 문제 → 줄 사이 간격을 넓혀 모든 칩이 44px를 유지하도록 수정(`elementFromPoint`로 실측).
6. **[도구]** 새 소스 파일의 정규식·문자열에 쓴 유니코드 이스케이프 표기(NUL·BOM·제어문자)가 저장 과정에서
   실제 제어문자 바이트로 바뀌어, git이 그 파일을 바이너리로 취급하던 것을 발견했다. 이스케이프 표기로 되돌리고
   수정·신규 파일 전체를 다시 검사했다(제어문자·BOM·깨진 문자 잔여 0건).

**자동 테스트**: `npm test` 100/100 → **129/129 통과**(신규 `compose.test.mjs`·`folder-color.test.mjs`·`folders.test.mjs`
+ 자정 결함 수정), `npm run test:syntax` 통과. 폴더 저장소 테스트는 가짜 Dexie로 **실제 `folders.js` 코드**를
돌리고, 백업 복원은 실제 `validateAndNormalize`를 호출한다.

**실제 브라우저(390×844, 320×568, 1180×820)로 직접 확인한 것**

- `Paste code`: 실제 클릭으로 창 열림 → 글 입력칸 자동 포커스 → HTML 입력 중 `HTML` 자동 선택 → `Add to library` →
  열려 있던 폴더에 저장, 제목 `Hello 폴리오`(`<title>`에서 추출), `Open` 토스트. Markdown·CSV·Text, 제목 직접 입력,
  `Unsorted` 선택, 중복(`That exact text is already in folio.`), 빈 입력 비활성, `Cancel`·`Esc` 후 초안 복원,
  클립보드 실패 안내, `Clear`. 붙여넣은 Markdown(제목·목록)·CSV(한글 셀)·텍스트가 앱 뷰어에서 정상 표시.
- 폴더: `+ New folder` → 자동 열림·폴더 바 표시·"비어 있음" 안내 → `Rename`(탭·문서 배지·폴더 바 동시 갱신, 중복
  이름 거부) → `Delete`(확인 창에 문서 수, 취소 시 유지, 삭제 후 `Unsorted` 복귀) → `⋯ → Folders` 시트의 이름 변경·삭제·생성도 정상.
  새로고침 후에도 폴더·열린 폴더 유지. 5개 폴더가 서로 다른 색.
- 요약 줄: `Pinned` 선택 시 `Showing 0 of 7`, 검색 시 `Showing 1 of 7`, `Clear filters`가 검색어까지 비움.
- 터치 영역: 모든 칩이 보이는 크기 30~32px에 **실제 탭 영역 44px**, 하단 버튼 44px. 글자 크기 17px·6px에서 가로 스크롤 없음·
  버튼 잘림 없음, `Paste code` 창의 입력창·선택창·글 입력칸은 16px 유지.
- 다크 모드에서 5색 탭·선택 상태(진한 채움 + 밝은 2px 테두리 + 굵은 글씨) 확인. 색 대비(글자색 기준): 라이트 4.99~9.7:1, 다크 4.85~12.2:1.
- iPad 가로(1180×820) 320px 라이브러리 레일에서 하단 바(118+118+44px) 넘침 없음.
- 보안: 폴더 이름·제목·본문에 `<img onerror>`·`<script>`를 넣어도 글자로만 표시되고 실행 0건.
- 백업 → 검증·복원 왕복에서 폴더 색 유지.
- 내장 브라우저는 Service Worker 등록을 못 하는 환경이라 콘솔에 `An unknown error occurred when fetching the script`
  오류가 페이지 로드마다 1건 남는다(변경과 무관, 기존과 동일). 이 환경은 **가져온 HTML 파일도 뷰어 본문이 비어 보이는**
  중첩 샌드박스 iframe 한계가 있어(붙여넣은 HTML과 파일로 가져온 HTML이 동일하게 동작) HTML 렌더링은 실기기 항목으로 둔다.

**Pending — 실기기 확인 필요**

- [ ] iPhone Safari에서 `Paste code` 창: 키보드가 열린 상태에서 `Add to library`(창 맨 위)가 가려지지 않는지, 텍스트 영역
  포커스 시 키보드가 자동으로 올라오는지, 긴 코드를 붙여넣었을 때 스크롤
- [ ] iOS에서 `Paste from clipboard` 버튼의 붙여넣기 허용 확인 동작(거부되면 안내 문구가 뜨는지)
- [ ] **붙여넣은 HTML 문서**가 `Read`/`Run`에서 정상 표시되는지(파일로 가져온 HTML과 같은 경로)
- [ ] 한글 IME로 `Paste code` 입력칸·제목칸에서 조합 중 종류 자동 선택·`Enter`가 오작동하지 않는지
- [ ] 두 기기에서 Sync를 켠 채 폴더 이름을 바꿔 다른 기기에 반영되는지(코드 경로와 단위 테스트로만 확인)
- [ ] 폴더 탭이 많을 때(7개 이상) 가로 스크롤과 열린 폴더 탭 자동 스크롤, iPad 세로·가로
- [ ] Add to Home Screen 앱에서 새 빌드로 갱신되는지(캐시 버전 `2026.09.18-pastefolders1`)
