...
### 뒤이은 2차 검토(같은 날) — 꼼꼼히 다시 확인해 결함 수정 (빌드 `2026.09.18-pastefolders2`)

첫 배포 직후 같은 변경을 처음부터 다시 검토했다(코드 재독, 실제 브라우저 조작, 접근성 트리, 글자 크기 6단계 전부,
가로·세로 화면, 악의적 입력). 아래는 이때 **새로 발견해 고친 것**이다.

1. **[신규 기능] 한글 조합 중 `Add to library`를 누르면 마지막 글자들이 저장되지 않음.** 입력칸의 "조합 중"
   `input` 이벤트를 건너뛰고 미러링한 값을 저장해서, 마지막 `input`이 조합 중이면 값이 한 음절 이상 뒤처졌다
   (iOS는 버튼을 탭해도 조합이 확정되지 않을 수 있음). 입력칸·제목칸의 현재 값을 직접 읽도록 수정. 조합 이벤트를
   흉내 낸 시험에서 수정 전에는 제목 `한`만 저장, 수정 후에는 본문·제목 전체 저장. 조합 중 `Enter`(제목칸)·`Ctrl+Enter`는 여전히 무시.
2. **[기존 결함] 칩의 실제 터치 영역이 글자 크기 6·8·10px에서 42px.** 투명 `::after`의 `inset`이 테두리를 뺀
   안쪽 상자 기준이라 최소 높이 30px 칩은 42px가 됐다(12px에서만 44px). 새 칩뿐 아니라 기존 상태 칩·태그 칩도 해당.
   `inset`을 `-8px`로 고침. 계산된 `::after` 높이로 6단계 전부 ≥44px, 위아래로 붙은 칩 줄 사이 **터치 영역 겹침 0건**
   (홈 화면과 `Paste code` 창 모두)을 확인.
3. **[신규 기능] 제목 추출이 특정 입력에서 멈춤.** `# a` 뒤에 공백 3만 개가 있는 줄에서 제목 정규식이 25초 넘게
   역추적(창이 얼어붙음). 선형 시간의 문자열 처리로 바꾸고, 앞 10만 자만 읽도록 제한(HTML의 태그 제거 대체 경로는 2만 자).
   악의적 입력 10종이 합쳐서 0.3초 이내, 큰 정상 문서(3MB)는 수십 ms. 함께 바로잡음: ``` 코드 블록 안의 `# 주석`이 제목이 되던 것,
   `# Learning C#`의 `#`이 지워지던 것.
4. **[기존 결함] 오프라인 사전 캐시에 `src/diagram.js`(Markdown 뷰어)·`src/expiry.js`(보관 기간)가 빠져 있었음.**
   Service Worker는 목록 밖 파일을 "온라인에서 처음 가져올 때"만 담으므로, 설치 직후나 업데이트 직후 온라인 실행 없이
   바로 오프라인으로 켜면 모듈 하나가 없어 앱 전체가 시작하지 못할 수 있었다. 목록에 추가하고, `src/`·`src/handlers/`의
   모든 모듈이 목록에 있는지·목록의 모든 파일이 실제로 있는지·`index.html`이 참조하는 파일이 모두 담기는지 검사하는
   테스트(`tests/precache.test.mjs`)를 추가.
5. **[기존 결함] 입력창에서 글자를 드래그해 선택하다가 어두운 배경에서 손을 떼면 시트가 닫힘.** 눌림이 **배경에서 시작한
   경우에만** 닫히도록 수정(모든 시트 공통). 배경을 직접 탭하면 여전히 닫히고, `Cancel`·`Esc`도 그대로.
6. **[접근성] 폴더 만들기·이름 바꾸기·삭제 뒤 키보드/스크린리더 포커스가 사라짐**(다시 그려지며 포커스 있던 버튼이 사라짐).
   이름 바꾸기 → `Rename`, 삭제 → `All` 탭, 만들기 → 새 폴더 탭으로 포커스를 돌려줌. `⋯ → Folders` 시트 안에서는 시트가 포커스를 유지.
7. **[화면] 폭 768px 이상(iPad 세로·가로)에서 칩 줄·폴더 바·요약 줄이 문서 목록(720px 열)보다 24px 왼쪽에 어긋남.**
   같은 720px 열로 정렬(분할 보기의 320px 레일에서는 전체 폭 그대로).
8. **[신규 기능] 폴더 이름을 `All`·`Unsorted`로 지을 수 있어 같은 이름의 탭이 둘 생기던 것** → 거부(`Pick another name — …`).
   Sync가 다른 기기에서 받아 오는 이름은 거부하지 않음.
9. 칩 줄의 가로 스크롤바를 `scrollbar-width: thin`으로(Continue 줄과 통일).

**추가로 확인한 것(문제 없음)**: 글자 크기 6·8·10·12·14·17px 전부에서 홈 화면·`Paste code` 창의 가로 스크롤·버튼 잘림 없음,
iPhone 가로(844×390)에서 창 맨 위 `Add to library` 바로 사용 가능, iPad 세로(768×1024)·가로 분할 보기(320px 레일),
320px 폭, 다크 모드, 60자 긴 폴더 이름(탭·폴더 바·배지·선택 창 모두 말줄임), 색 번호 없는 옛 폴더(폴더 id에서 정해진 색),
폴더 12개(가로 스크롤·색 반복), 접근성 트리(폴더 탭 `Folder 2, 1 document`, 그룹 `Selected folder` + `Rename folder …`,
`status "Showing 0 of 1"`, 대화상자 이름·모든 컨트롤 이름), 붙여넣기 창의 초점 순서(Add → Cancel → 제목 → 종류 → 폴더 → 글 → 붙여넣기 → Clear).

**자동 테스트**: `npm test` 129/129 → **137/137 통과**(신규: 제목 추출 회귀·악의적 입력 3건, 사전 캐시 4건, 예약 이름 1건),
`npm run test:syntax` 통과.

**Pending — 실기기 확인 필요** (위 첫 목록에 더해)

- [ ] 실제 iOS 한글 키보드로 `Paste code`에서 글을 쓰다가 마지막 음절이 조합 중일 때 `Add to library`를 눌러도 끝까지 저장되는지
