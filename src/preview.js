...
export function instrument(session) {
  return '<scr' + 'ipt>(function(){var S=' + JSON.stringify(session) + ';var fsBase=null;function p(t,d){try{parent.postMessage(Object.assign({__folioPreview:1,session:S,type:t},d||{}),"*")}catch(e){}}'
    ...
    + 'var HLC=["core","agree","question","word","quote"];var HLITEMS=[];function applyHL(items){HLITEMS=(items||[]).slice();if(!window.CSS||!CSS.highlights||typeof Highlight!=="function")return;for(var i=0;i<HLC.length;i++)CSS.highlights.delete("folio-"+HLC[i]);var groups={};for(var k=0;k<HLC.length;k++)groups[HLC[k]]=[];(items||[]).forEach(function(item){var rg=findRange(item.quote,item.prefix||"",item.suffix||"");if(rg)groups[HLC.indexOf(item.color)>=0?item.color:"core"].push(rg)});for(var m=0;m<HLC.length;m++){if(groups[HLC[m]].length)CSS.highlights.set("folio-"+HLC[m],new Highlight(...groups[HLC[m]]))}}'
    // Hit-tests a tap point against the SAME ranges applyHL just drew, so
    // tapping an existing highlight can open its note editor even though this
    // document's DOM never leaves the sandbox (see the click listener below,
    // and reportFrameHighlightTap in app.js which matches the posted
    // quote/prefix/suffix back to the real stored annotation).
 ...
 * @param {object} options {html, session, allowScripts, innerSandbox, title, restoreY, onIssue, onScroll, onOpen, onOpenAsset, onTap}
...
export function mount(container, options) {
  ...
  // iOS Safari gives a tapped iframe keyboard focus, and the FIRST tap on
  // anything outside it afterward only blurs the iframe rather than also
  // registering as a click on that outside element — a well-known WebKit
  // quirk, not a folio bug, but this app's own "tap the page" gesture
  // (reportFrameTap, html.js) means the reader taps INSIDE this iframe
  // constantly, so every topbar button next to it (the "Aa" text-size
  // button in particular) would need two taps right after. tabIndex=-1
  // keeps the iframe out of the focus chain entirely, since nothing here
  // needs it focusable (all interaction is touch/postMessage, never Tab).
  frame.tabIndex = -1;
  ...
  function onMessage(event) {
    ...
    } else if (data.type === 'highlight-tap') {
      if (options.onHighlightTap) {
        options.onHighlightTap({
          quote: String(data.quote || ''),
          prefix: String(data.prefix || ''),
          suffix: String(data.suffix || ''),
          color: String(data.color || 'core'),
        });
      }
